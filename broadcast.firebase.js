...
import { startFiltersWatcher, stopFiltersWatcher, getFiltersOnce, applyFiltersToText } from './filters.js';
...
// Ensure Firestore and filters listeners are cleaned up when the page is unloaded.
// This prevents memory leaks and dangling snapshot listeners if the user
// navigates away.  We explicitly call stopFiltersWatcher() to cancel
// the underlying onSnapshot subscription in filters.js.  Additionally we
// unsubscribe from the broadcast feed listener if present.
window.addEventListener('beforeunload', () => {
  try { stopFiltersWatcher(); } catch {}
...
sendBtn?.addEventListener('click', async () => {
  ...
  if (honeyEl && honeyEl.value && honeyEl.value.trim() !== '') {
    toast('Blocked (bot detected).', 'error');
    return;
  }

  // cool-down
  if (msLeft() > 0) {
    toast('Not so fast — wait a few seconds.', 'error');
    return;
  }
  ...
  if (getDailyCount() >= DAILY_CAP) {
    toast('Daily limit reached. Try tomorrow.', 'error');
    return;
  }
  ...
  if (lastPack && (now() - (lastPack.t || 0) < DUP_WINDOW_MS) && (lastPack.v || '') === text) {
    toast('Duplicate too soon.', 'error');
    return;
  }
